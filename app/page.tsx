...
function EditableAmt({ value, onChange, color, size = "md" }: { value: number; onChange: (n: number) => void; color?: string; size?: "sm" | "md" | "lg"; }) {
  ...
  const debounceRef = useRef<ReturnType<typeof setTimeout>>(null); function commit() { const n = parseFloat(draft); if (!isNaN(n) && n >= 0) { if (debounceRef.current) clearTimeout(debounceRef.current); debounceRef.current = setTimeout(() => onChange(n), 400); } setEditing(false); }
...
export default function BudgetApp() {
  ...
  async function handleAuth() {
    ...
    try {
      ...
      if (d.success && d.token) {
        ...
        await loadData();
        if (d.needs_onboarding) { setNeedsOnboarding(true); fetch("/api/budget/health", { headers: getAuthHeaders() }).catch(() => {}); } else { /* tour handled separately */ }
      } else {
  ...
  if (m) {
    ...
    Object.entries(ba).forEach(([k, alloc]) => {
      if (alloc > 0 && bv[k]) {
        inAppAlerts.push({ type: "info", id: `env-${k}`, title: `Reste à vivre ${k} validée`, detail: `${alloc.toFixed(0)} EUR` });
      }
    });
  }
  // Forecast alerts
      ...
      <header className="app-header" style={{ background: S.surface, borderBottom: `1px solid ${S.border}`, display: "flex", alignItems: "stretch", position: "sticky", top: 0, zIndex: 40 }}>
        ...
        <div className="header-tools" style={{ display: "flex", alignItems: "center", gap: 5, padding: "0 14px", borderLeft: `1px solid ${S.border}` }}>
          ...
          <div style={{ position: "relative" }}>
            ...
            {showAlerts && <div style={{ position: isMobile ? "fixed" as any : "absolute" as any, top: isMobile ? 50 : 38, right: isMobile ? 10 : 0, left: isMobile ? 10 : "auto", width: isMobile ? "auto" : 300, maxHeight: isMobile ? "60vh" : 380, overflowY: "auto" as const, background: S.surface, border: "none", borderRadius: 16, boxShadow: "0 12px 40px rgba(0,0,0,0.15)", zIndex: 100, padding: 8 }}>
...
function ProjectionTab({ forecast: f, prevCumul = 0, goalMonthly = 0, allMonths = [] }: { forecast: Forecast; prevCumul?: number; goalMonthly?: number; allMonths?: Month[] }) {
...
function HistoriqueTab({ months, goalMonthly = 0 }: { months: Month[]; goalMonthly?: number }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      {/* Taux d'effort par catégorie — données passées */}
      <Card>
        <SLabel>Taux d&apos;effort par catégorie</SLabel>
        <p style={{ fontSize: 10, color: S.muted, margin: "-4px 0 8px" }}>Basé sur vos {months.length} mois enregistrés</p>
        {(() => { const src = months; const fixedT = src.reduce((s, m) => s + m.expenses.filter(e => e.category === 'fixed').reduce((s2, e) => s2 + e.amount, 0), 0); const varT = src.reduce((s, m) => s + m.expenses.filter(e => e.category === 'variable').reduce((s2, e) => s2 + e.amount, 0), 0); const savT = src.reduce((s, m) => s + m.expenses.filter(e => e.category === 'investment').reduce((s2, e) => s2 + e.amount, 0), 0); const allocKeys = [...new Set(src.flatMap(m => Object.keys(m.budget_allocation || {})))]; const mergeKeys = ['revolut','amex','cera']; const otherKeys = allocKeys.filter(k => !mergeKeys.includes(k.toLowerCase())); const otherCats = otherKeys.map(k => ({ n: k.charAt(0).toUpperCase() + k.slice(1), v: src.reduce((s, m) => s + (((m.budget_allocation as any) || {})[k] || 0), 0), c: S.muted })); const mergedVal = allocKeys.filter(k => mergeKeys.includes(k.toLowerCase())).reduce((s, k) => s + src.reduce((s2, m) => s2 + (((m.budget_allocation as any) || {})[k] || 0), 0), 0); const allocCats = [...otherCats, ...(mergedVal > 0 ? [{ n: 'Autres', v: mergedVal, c: S.muted }] : [])]; const cats: {n:string;v:number;c:string}[] = [{n:'Fixes',v:fixedT,c:S.primary},{n:'Variables',v:varT,c:'#8b5cf6'},{n:'Épargne',v:savT,c:S.accent}, ...allocCats]; const objectives: Record<string,number> = {'Fixes':25,'Variables':15,'Épargne':20,'Courses':7,'Restaurants':10,'Services':3,'Autres':20}; const inObj = cats.filter(cat => { const pct = ti > 0 ? Math.round(cat.v / ti * 100) : 0; const obj = objectives[cat.n]; if (!obj) return true; return cat.n === 'Épargne' ? pct >= obj : pct <= obj; }).length; return (<>{cats.map(cat => { const pct = ti > 0 ? Math.round(cat.v / ti * 100) : 0; const obj = objectives[cat.n] || 0; const isEp = cat.n === 'Épargne'; const ok = obj > 0 ? (isEp ? pct >= obj : pct <= obj) : true; const sc = ok ? S.success : S.danger; return (<div key={cat.n} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}><span style={{ fontSize: 10, fontWeight: 600, color: S.muted, width: 80 }}>{cat.n}</span><div style={{ flex: 1, height: 10, background: S.surface2, borderRadius: 5, position: "relative" as const }}><div style={{ height: "100%", width: `${Math.min(100, pct)}%`, background: cat.c, borderRadius: 5 }} />{obj > 0 && <div style={{ position: "absolute" as const, top: -3, bottom: -3, left: `${obj}%`, width: 2, background: S.text, borderRadius: 1, opacity: 0.5 }}><div style={{ position: "absolute" as const, top: -5, left: "50%", transform: "translateX(-50%)", width: 0, height: 0, borderLeft: "3px solid transparent", borderRight: "3px solid transparent", borderTop: `4px solid ${S.text}`, opacity: 0.5 }} /></div>}</div><span style={{ fontSize: 10, fontWeight: 700, color: sc, width: 70, textAlign: "right" as const, whiteSpace: "nowrap" as const }}>{pct}% <span style={{ fontSize: 8, fontWeight: 500, opacity: 0.5, color: S.muted }}>/ {obj}%</span></span><div style={{ width: 6, height: 6, borderRadius: 3, background: sc, flexShrink: 0 }} /></div>); })}<div style={{ textAlign: "center" as const, marginTop: 8 }}><span style={{ fontSize: 9, fontWeight: 600, color: inObj >= 5 ? S.success : S.warning, background: `${inObj >= 5 ? S.success : S.warning}15`, padding: "2px 10px", borderRadius: 6 }}>{inObj}/{cats.length} dans l&apos;objectif</span></div></>); })()}
      </Card>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <Card style={{ borderColor: `${S.accent}25` }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 8 }}>
          {m.expenses.filter((e: Expense) => e.category === "investment").map((e: Expense) => (
            <div key={e.label} className="row-h" style={{ display: "flex", alignItems: "center", gap: 8, padding: "9px 12px", background: e.validated ? `${S.accent}12` : S.surface2, borderRadius: 10, border: `1px solid ${e.validated ? S.accent + "40" : S.border}` }}>
              ...
              <span onClick={() => { const v = prompt("Nouveau montant :", String(e.amount)); if (v !== null) { const amt = parseFloat(v); if (!isNaN(amt)) { fetch(`/api/budget/month/${m.month_key}/expense`, { method: "PATCH", headers: getAuthHeaders(), body: JSON.stringify({ label: e.label, amount: amt }) }).then(() => window.location.reload()); } } }} className="editable-amt" style={{ fontFamily: S.heading, fontSize: 14, fontWeight: 700, color: S.accent, cursor: "pointer" }}>{fmt(e.amount)}</span>
        ...
        <button onClick={async () => { const mi = months.findIndex(mo => mo.month_key === m.month_key); let prev = mi > 0 ? months[mi-1] : null; if (!prev && mi === 0) { const [y] = m.month_key.split("-").map(Number); try { const pr = await fetch(`/api/budget/month/${y-1}-12`, { headers: getAuthHeaders() }); if (pr.ok) prev = await pr.json(); } catch {} } if (!prev) { alert("Pas de mois précédent"); return; } const prevInv = prev.expenses.filter((e: any) => e.category === "investment"); if (prevInv.length === 0) { alert("Aucun investissement le mois précédent"); return; } if (!confirm(`Copier ${prevInv.length} investissements de ${prev.month_name} ?`)) return; prevInv.forEach((e: any) => { if (!m.expenses.some((ex: any) => ex.label === e.label) && onAddInvestment) onAddInvestment(e.label, e.amount); }); }} style={{ marginTop: 4, display: "flex", alignItems: "center", gap: 6, padding: "6px 14px", fontSize: 11, fontWeight: 600, border: `1px solid ${S.accent}25`, borderRadius: 8, background: `${S.accent}08`, color: S.accent, cursor: "pointer", width: "100%", justifyContent: "center", fontFamily: S.font }}>Copier M-1</button>
      </Card>
      ...
      <Card>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, flexWrap: "wrap" as const, gap: 8 }}>
          ...
          <div style={{ display: "flex", gap: 6 }}>
            <button onClick={async () => { const r = await fetch(`/api/budget/export-portfolio-csv?year=${months[0]?.year || 2026}`, { headers: getAuthHeaders() }); const d = await r.json(); if (d.csv) { const blob = new Blob([d.csv], { type: "text/csv" }); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = `portefeuille_${d.year}.csv`; a.click(); URL.revokeObjectURL(url); } }} style={{ fontSize: 10, fontWeight: 600, color: S.primary, background: `${S.primary}10`, border: `1px solid ${S.primary}30`, borderRadius: 6, padding: "3px 10px", cursor: "pointer", fontFamily: S.font }}>Exporter CSV</button>
            <button onClick={() => { const input = document.createElement("input"); input.type = "file"; input.accept = ".csv"; input.onchange = async (e) => { const file = (e.target as HTMLInputElement).files?.[0]; if (!file) return; const text = await file.text(); if (!confirm("Importer dans le portefeuille ?")) return; const r = await fetch("/api/budget/import-portfolio-csv", { method: "POST", headers: getAuthHeaders(), body: JSON.stringify({ csv: text, year: months[0]?.year || 2026 }) }); const d = await r.json(); if (r.ok) { alert("Import OK : " + d.parsed_rows + " lignes"); window.location.reload(); } else { alert(d.detail || "Erreur"); } }; input.click(); }} style={{ fontSize: 10, fontWeight: 600, color: S.accent, background: `${S.accent}10`, border: `1px solid ${S.accent}30`, borderRadius: 6, padding: "3px 10px", cursor: "pointer", fontFamily: S.font }}>Importer CSV</button>
          </div>
          {totalValue > 0 && (
