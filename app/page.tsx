    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <Card>
          ...
          {(() => { const fixedT = allMonths.reduce((s, m) => s + m.expenses.filter(e => e.category === 'fixed').reduce((s2, e) => s2 + e.amount, 0), 0); const varT = allMonths.reduce((s, m) => s + m.expenses.filter(e => e.category === 'variable').reduce((s2, e) => s2 + e.amount, 0), 0); const savT = allMonths.reduce((s, m) => s + m.expenses.filter(e => e.category === 'investment').reduce((s2, e) => s2 + e.amount, 0), 0); const allocKeys = [...new Set(allMonths.flatMap(m => Object.keys(m.budget_allocation || {})))]; const allocCats = allocKeys.map(k => ({ n: k.charAt(0).toUpperCase() + k.slice(1), v: allMonths.reduce((s, m) => s + (((m.budget_allocation as any) || {})[k] || 0), 0), c: S.muted })); const cats: {n:string;v:number;c:string}[] = [{n:'Fixes',v:fixedT,c:S.primary},{n:'Variables',v:varT,c:'#8b5cf6'},{n:'Épargne',v:savT,c:S.accent}, ...allocCats]; return cats.map(cat => (<div key={cat.n} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}><span style={{ fontSize: 10, fontWeight: 600, color: S.muted, width: 80 }}>{cat.n}</span><div style={{ flex: 1, height: 8, background: S.surface2, borderRadius: 4, overflow: "hidden" }}><div style={{ height: "100%", width: `${ti > 0 ? Math.min(100, Math.round(cat.v / ti * 100)) : 0}%`, background: cat.c, borderRadius: 4 }} /></div><span style={{ fontSize: 10, fontWeight: 700, color: cat.c, width: 35, textAlign: "right" as const }}>{ti > 0 ? Math.round(cat.v / ti * 100) : 0}%</span></div>)); })()}
        </Card>
