    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div style={{ display: "flex", gap: 0, background: S.surface, borderRadius: 14, border: `1px solid ${S.border}`, overflow: "hidden" }}>
        {[
          ...
          { label: "Épargne", value: (m.savings?.target_monthly ?? 140) + m.expenses.filter((e: Expense) => e.category === "investment").reduce((s: number, e: Expense) => s + e.amount, 0) , color: S.accent, icon: "★", tip: "Objectif épargne + investissements mensuels", sub: `Cumul: ${fmt((m.savings?.cumulative_target ?? 0) + months.slice(0, idx + 1).reduce((s: number, mo: Month) => s + mo.expenses.filter((e: Expense) => e.category === "investment").reduce((s2: number, e2: Expense) => s2 + e2.amount, 0), 0))}` },
        ].map((k, i, arr) => (
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <Card style={{ borderColor: `${S.primary}20` }}>
        ...
        <div style={{ display: "flex", gap: 12, padding: 12, background: S.surface2, borderRadius: 10, marginBottom: 14 }}>
          ...
          <div style={{ flex: 1, textAlign: "center" }}><div style={{ fontSize: 9, color: S.muted, textTransform: "uppercase" as const }}>Réalisé</div><div style={{ fontFamily: S.heading, fontSize: 18, fontWeight: 800, color: S.success }}>{fmt(sav.actual_monthly)}</div></div>
          <div style={{ width: 1, background: S.border }}></div>
