    ...
    <div style={{ background: S.bg, minHeight: "100vh", color: S.text, fontFamily: S.font, "--mob-bg": S.surface, "--mob-border": S.border, "--mob-muted": S.muted, "--mob-accent": S.accent } as React.CSSProperties}>
      ...
      <header className="app-header" style={{ background: S.surface, borderBottom: `1px solid ${S.border}`, display: "flex", alignItems: "stretch", position: "sticky", top: 0, zIndex: 40 }}>
        ...
        <div className="header-tools" style={{ display: "flex", alignItems: "center", gap: 5, padding: "0 14px", borderLeft: `1px solid ${S.border}` }}>
          <button onClick={() => setNeedsOnboarding(true)} title="Aide" style={{ display: isMobile ? "none" : "flex", width: 30, height: 30, borderRadius: 8, border: `1px solid ${S.border}`, background: S.bg, color: S.muted, alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: 13, fontWeight: 700 }}>?</button>
          <div style={{ position: "relative" }}>
            ...
            {showAlerts && <div style={{ position: isMobile ? "fixed" as any : "absolute" as any, top: isMobile ? 50 : 38, right: isMobile ? 10 : 0, left: isMobile ? 10 : "auto", width: isMobile ? "auto" : 300, maxHeight: 380, overflowY: "auto", background: S.surface, border: `1px solid ${S.border}`, borderRadius: 12, boxShadow: "0 8px 32px rgba(0,0,0,0.12)", zIndex: 100, padding: 8 }}>
              <div style={{ padding: "8px 10px 6px", fontFamily: S.heading, fontSize: 13, fontWeight: 700, borderBottom: `1px solid ${S.border}`, marginBottom: 4 }}>Notifications</div>
              {inAppAlerts.filter(a => !dismissedAlerts.includes(a.id)).length === 0 && <div style={{ padding: "16px 10px", textAlign: "center", color: S.muted, fontSize: 12 }}>Tout est en ordre !</div>}
              {inAppAlerts.filter(a => !dismissedAlerts.includes(a.id)).map((a) => { const cl = a.type === "danger" ? S.danger : a.type === "warning" ? S.warning : a.type === "success" ? S.success : a.type === "insight" ? S.primary : S.accent; return (<div key={a.id} style={{ display: "flex", gap: 8, alignItems: "flex-start", padding: "8px 10px", borderRadius: 8, marginBottom: 3, background: `${cl}08`, border: `1px solid ${cl}15` }}><div style={{ width: 24, height: 24, borderRadius: 7, background: `${cl}15`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 1 }}>{a.type === "success" ? <Check size={11} color={cl} /> : a.type === "action" ? <Bell size={11} color={cl} /> : <AlertTriangle size={11} color={cl} />}</div><div style={{ flex: 1 }}><div style={{ fontSize: 11, fontWeight: 700, color: cl }}>{a.title}</div><div style={{ fontSize: 10, color: S.muted, marginTop: 1, lineHeight: 1.4 }}>{a.detail}</div>{a.actionLabel && <button onClick={() => { setTab((a.actionTab || "dashboard") as any); setShowAlerts(false); }} style={{ fontSize: 9, fontWeight: 700, color: S.accent, background: "none", border: "none", padding: "3px 0", cursor: "pointer", fontFamily: S.font }}>{a.actionLabel} &rarr;</button>}</div><button onClick={() => dismissAlert(a.id)} style={{ background: "none", border: "none", color: S.muted, fontSize: 11, cursor: "pointer", opacity: 0.3, padding: 0, flexShrink: 0 }}>x</button></div>); })}
            </div>}
          </div>
