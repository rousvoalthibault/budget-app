        ...
        <div className="header-tools" style={{ display: "flex", alignItems: "center", gap: 5, padding: "0 14px", borderLeft: `1px solid ${S.border}` }}>
          ...
          <div style={{ position: "relative" }}>
            ...
            {showAlerts && <div style={{ position: isMobile ? "fixed" as any : "absolute" as any, top: isMobile ? 50 : 38, right: isMobile ? 10 : 0, left: isMobile ? 10 : "auto", width: isMobile ? "auto" : 300, maxHeight: 380, overflowY: "auto", background: S.surface, border: `1px solid ${S.border}`, borderRadius: 12, boxShadow: "0 8px 32px rgba(0,0,0,0.12)", zIndex: 100, padding: 8 }}>
              ...
              {inAppAlerts.filter(a => !dismissedAlerts.includes(a.id)).map((a) => { const bgMap: Record<string,string> = { danger: "linear-gradient(135deg, #fef2f2, #fff7ed)", warning: "linear-gradient(135deg, #fffbeb, #fff7ed)", success: "linear-gradient(135deg, #f0fdf4, #ecfdf5)", insight: "linear-gradient(135deg, #eff6ff, #f0fdf4)", action: "linear-gradient(135deg, #fff7ed, #fffbeb)" }; const borderMap: Record<string,string> = { danger: "#fecaca", warning: "#fed7aa", success: "#bbf7d0", insight: "#bfdbfe", action: "#fed7aa" }; const cl = a.type === "danger" ? S.danger : a.type === "success" ? S.success : a.type === "insight" ? S.primary : a.type === "warning" ? S.warning : S.accent; return (<div key={a.id} style={{ marginBottom: 8 }}><div style={{ padding: "10px 14px", borderRadius: 16, borderBottomLeftRadius: 4, background: bgMap[a.type] || bgMap.action, border: `1px solid ${borderMap[a.type] || borderMap.action}`, fontSize: 12, lineHeight: 1.5, color: "#334155" }}><strong style={{ color: cl }}>{a.title}</strong><span style={{ color: "#64748b" }}> — {a.detail}</span></div><div style={{ display: "flex", gap: 8, marginTop: 3, paddingLeft: 4 }}>{a.actionLabel && <button onClick={() => { setTab((a.actionTab || "dashboard") as any); setShowAlerts(false); }} style={{ fontSize: 10, fontWeight: 700, color: S.primary, background: "none", border: "none", cursor: "pointer", fontFamily: S.font, padding: 0 }}>{a.actionLabel} →</button>}<button onClick={() => dismissAlert(a.id)} style={{ fontSize: 9, color: "#cbd5e1", background: "none", border: "none", cursor: "pointer", fontFamily: S.font, padding: 0 }}>Ignorer</button></div></div>); })}
            </div>}
          </div>
