...
export default function BudgetApp() {
  ...
  async function patchExpense(mk: string, label: string, updates: Partial<Expense>) {
    ...
    try {
      ...
      if (d.success) {
        ...
        if (updates.validated !== undefined) showToast(updates.validated ? `Ok - ${label}` : `${label} devalidee`);
  ...
  async function patchBudgetAlloc(mk: string, updates: { amounts?: Record<string, number>; validated?: Record<string, boolean> }) {
    try {
      const r = await fetch(`/api/budget/month/${mk}/budget-allocation`, {
        method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(updates),
      });
      const d = await r.json();
      if (d.success) {
        setMonths(prev => prev.map(m => m.month_key !== mk ? m : {
          ...m,
          budget_allocation: updates.amounts ? { ...m.budget_allocation, ...updates.amounts } as BudgetAlloc : m.budget_allocation,
          budget_validated: updates.validated ? { ...(m.budget_validated || {}), ...updates.validated } : m.budget_validated,
        }));
  ...
  const m = months[idx];
  const validatedBudget = m
    ? Object.entries(m.budget_validated || {}).filter(([, v]) => v)
        .reduce((sum, [k]) => sum + ((m.budget_allocation as unknown as Record<string, number>)[k] || 0), 0)
    : 0;
  ...
  return (
    <div style={{ background: S.bg, minHeight: "100vh", color: S.text, fontFamily: S.font }}>
      ...
      <main key={tab} style={{ padding: "24px", maxWidth: 1200, margin: "0 auto", animation: "fadeUp 0.25s ease" }}>
        {tab === "dashboard" && m && (
          <DashboardTab month={m} netBalance={netBal} totalExpenses={totalExp} validatedBudget={validatedBudget}
            validatedCount={validatedCnt} totalCount={totalItems}
        ...
        {tab === "economies" && months.length > 0 && (
          <EconomiesTab months={months} currentIdx={idx} onSavingsChange={(mk, u) => patchSavings(mk, u)} />
        )}
      </main>
    </div>
  );
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 16 }}>
        {[
          ...
          { label: "Objectif economies", value: savTarget, color: S.accent, editable: false, sub: `Cumul: ${fmt(m.savings?.cumulative_target ?? 0)}` },
      ...
      <Card>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          ...
          <span style={{ color: validatedCount === totalCount ? S.success : S.muted, fontSize: 13, fontWeight: 700 }}>
            {validatedCount === totalCount ? "Tout valide !" : `${validatedCount} / ${totalCount}`}
          </span>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
        {[
          { label: "Depenses fixes", items: fixed, color: S.primary },
          { label: "Investissements", items: invest, color: S.accent },
          { label: "Depenses variables", items: variable, color: S.warning },
            ...
            {pending.slice(0, 8).map(e => {
              ...
              return (
                <div key={e.label} className="row-h" style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", background: S.surface2, borderRadius: 10 }}>
                  ...
                  <button className="val-btn" onClick={() => onValidate(e.label, true)} disabled={saving === e.label} style={{ background: S.success, color: "#fff", border: "none", borderRadius: 7, padding: "4px 12px", fontSize: 12, fontFamily: S.font, fontWeight: 700, opacity: saving === e.label ? 0.5 : 1 }}>
                    Valider
                  </button>
                </div>
              );
            })}
            {pending.length > 8 && <p style={{ color: S.muted, fontSize: 12, textAlign: "center", margin: "4px 0 0" }}>+ {pending.length - 8} autres - voir onglet Depenses</p>}
          </div>
        </Card>
      )}
    </div>
...
// ── Depenses ──────────────────────────────────────────────────────────────────
function DepensesTab({ month: m, onValidate, onAmountChange, onIncomeChange, onBudgetChange, saving }: {
  month: Month;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <Card style={{ borderColor: `${S.success}30`, background: `linear-gradient(135deg, ${S.success}10, ${S.bg})` }}>
        ...
        <div style={{ display: "flex", gap: 32, flexWrap: "wrap" as const, alignItems: "center" }}>
          ...
          <div style={{ marginLeft: "auto", textAlign: "right" }}>
            <p style={{ color: S.muted, fontSize: 12, margin: "0 0 4px" }}>Total</p>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        <ColCard title="Depenses fixes" items={fixed} color={S.primary} />
        <ColCard title="Investissements & epargne" items={invest} color={S.accent} />
      </div>
      ...
      {/* Budget envelopes — validatable */}
      <Card style={{ borderColor: `${S.primary}25` }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
          <SLabel>Budgets enveloppes — valider = comptes dans les depenses</SLabel>
          {validatedBudgetTotal > 0 && (
            <span style={{ fontFamily: S.heading, fontSize: 16, color: S.primary, fontWeight: 700 }}>+{fmt(validatedBudgetTotal)} integres</span>
          )}
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 8 }}>
          {Object.entries(m.budget_allocation).map(([key, amount]) => {
            const isValid = budgetValidated[key] || false;
            const label = BUDGET_LABELS[key] || key;
            return (
              <div key={key} className="row-h" style={{ display: "flex", alignItems: "center", gap: 8, padding: "9px 12px", background: isValid ? `${S.primary}12` : S.surface2, borderRadius: 10, border: `1px solid ${isValid ? S.primary + "40" : S.border}`, transition: "all 0.2s" }}>
                ...
                <span style={{ flex: 1, fontSize: 13, color: isValid ? S.text : S.muted, fontWeight: 600 }}>{label}</span>
                ...
                <button className="val-btn"
                  onClick={() => onBudgetChange({ validated: { [key]: !isValid } })}
                  style={{ width: 28, height: 28, borderRadius: 7, border: `1.5px solid ${isValid ? S.primary : S.muted}`, background: isValid ? S.primary : "transparent", color: isValid ? "#fff" : S.muted, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}
                  title={isValid ? "Devalider" : "Valider - ajouter aux depenses"}>
                  <Check size={12} />
                </button>
              </div>
            );
...
function ProjectionTab({ forecast: f }: { forecast: Forecast }) {
  const data = f.months.map(m => ({ name: m.month_name.slice(0, 3), "Revenu": m.income, "Depenses": m.expenses, "Solde": m.balance, alert: m.alert_type }));
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(210px, 1fr))", gap: 16 }}>
        {[
          { l: "Revenu annuel", v: f.total_income, c: S.success },
          { l: "Depenses totales", v: f.total_expenses, c: S.danger },
                  ...
                  <tr key={mo.month_key} className="row-h" style={{ borderBottom: `1px solid ${S.border}` }}>
                    ...
                    <td style={{ padding: "9px 14px" }}>
                      <span style={{ background: `${ac}20`, color: ac, border: `1px solid ${ac}50`, borderRadius: 6, padding: "3px 10px", fontSize: 11, fontWeight: 700 }}>
                        {mo.alert_type === "danger" ? "NEGATIF" : mo.alert_type === "warning" ? "SERRE" : "OK"}
                      </span>
                    </td>
                  </tr>
                );
...
function EconomiesTab({ months, currentIdx, onSavingsChange }: {
  ...
  // Show ALL lines (including 0) as requested
  const portVals = portfolio.map(p => ({ ...p, value: (sav[p.key] as number) ?? 0 })).sort((a, b) => b.value - a.value);
  const totalPort = portVals.reduce((s, p) => s + p.value, 0);

  const chartData = months.map(mo => ({ name: mo.month_name.slice(0, 3), "Objectif": mo.savings.target_monthly, "Realise": mo.savings.actual_monthly }));
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <Card style={{ background: `linear-gradient(135deg, ${S.accent}12, ${S.bg})`, borderColor: `${S.accent}30` }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: S.muted }}>
          <span>{fmt(sav.cumulative_actual)} epargnes</span>
      ...
      <Card>
        ...
        <div style={{ display: "flex", gap: 32, flexWrap: "wrap" as const, alignItems: "center" }}>
          ...
          <div>
            <p style={{ color: S.muted, fontSize: 12, margin: "0 0 4px" }}>Realise ce mois</p>
              ...
              return (
                <div key={p.key as string}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4, alignItems: "center" }}>
                    <span style={{ fontSize: 13, color: p.value > 0 ? S.text : S.muted }}>{p.label}</span>
                  ...
                  <div style={{ background: S.surface2, borderRadius: 999, height: 5, overflow: "hidden" }}>
                    <div style={{ background: p.color, height: "100%", width: `${pct2}%`, borderRadius: 999, transition: "width 0.5s", opacity: p.value > 0 ? 1 : 0.3 }} />
                  </div>
